...
def deletar_flashcard(request, id):
    ...
    if not flashcard.user == request.user:
        messages.add_message(
        request, constants.ERROR, 'Você não pode realizar esta ação!')
        return redirect('/flashcard/novo_flashcard')
    else:
        flashcard.delete()
        messages.add_message(
...
def iniciar_desafio(request):
    if request.method == 'GET':
        ...
        if qtd_perguntas == '':
            messages.add_message(request,constants.ERROR,'Preencha o campo de quantidade de perguntas!',)
            return redirect('/flashcard/iniciar_desafio')
        elif titulo == '': 
            messages.add_message(request,constants.ERROR,'Preencha o campo de titulo!',)
            return redirect('/flashcard/iniciar_desafio')
        elif categorias == '':
            messages.add_message(request,constants.ERROR,'Defina as categorias!',)
            return redirect('/flashcard/iniciar_desafio')
        elif dificuldade == '':
            messages.add_message(request,constants.ERROR,'Defina a dificuldade!',)
            return redirect('/flashcard/iniciar_desafio')
        ...
        if flashcards.count() < int(qtd_perguntas):
            messages.add_message(
            request, constants.ERROR, 'Não há flashcards suficientes')
            return redirect('/flashcard/iniciar_desafio')
...
def listar_desafio(request):
    ...
    categoria_filtrar = request.GET.get('categoria')
    dificuldade_filtrar = request.GET.get('dificuldade')
    status = request.GET.get('respondido')
...
def relatorio(request, id):
    ...
    #Fazer o ranking por categorias
